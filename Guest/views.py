...
def homepage(request):
    all_location = Guesthouse.objects.values_list('location','id').distinct().order_by()
    if request.method =="POST":
        try:
            print(request.POST)
            guest = Guesthouse.objects.all().get(id=int(request.POST['search_location']))
            rr = []
            ...
            room = Rooms.objects.all().filter(guest=guest,capacity__gte = int(request.POST['capacity'])).exclude(id__in=rr)
            if len(room) == 0:
...
def panel(request):
    ...
    guest = Guesthouse.objects.values_list('location','id').distinct().order_by()

    response = render(request,'staff/panel.html',{'location':guest,'reserved':reserved,'rooms':rooms,'total_rooms':total_rooms,'available':available_rooms,'unavailable':unavailable_rooms})
    return HttpResponse(response)
...
def edit_room(request):
    ...
    if request.method == 'POST' and request.user.is_staff:
        ...
        guest = Guesthouse.objects.all().get(id=int(request.POST['guest']))
...
def add_new_location(request):
    if request.method == "POST" and request.user.is_staff:
        ...
        guesthouse = Guesthouse.objects.all().filter(location = location , state = state)
        if guesthouse:
            messages.warning(request,"Sorry City at this Location already exist")
            return redirect("staffpanel")
        else:
            new_guest = Guesthouse()
            new_guest.owner = owner
            new_guest.location = location
            new_guest.state = state
            new_guest.country = country
            new_guest.save()
...
def cancel_booking(request):
    book_id = request.GET['bookingid']
    booking = Reservation.objects.get(id=book_id)
    booking.delete()
    messages.success(request,"Booking canceled Successfully")
    return HttpResponse(render(request,'staff/allbookings.html',{'booking':booking}))